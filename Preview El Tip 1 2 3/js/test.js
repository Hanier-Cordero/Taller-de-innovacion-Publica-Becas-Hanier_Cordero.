...
function printReport(){
  ...
  <div class="report-intro"><div class="data"><div><strong>Participante:</strong> ${escapeHtml(name)}</div><div><strong>Correo:</strong> ${escapeHtml(email)}</div><div><strong>Departamento:</strong> ${escapeHtml(dep)}</div><div><strong>Municipio:</strong> ${escapeHtml(mun)}</div><div><strong>Fecha:</strong> ${new Date().toLocaleDateString('es-GT')}</div></div>
  <div class="orientation-note"><strong style="display:inline;margin:0">Nota:</strong> Las carreras se seleccionan según la afinidad de tus respuestas y la priorización institucional. Este instrumento es una guía inicial rápida basada en el test CHASIDE y el modelo de John Holland (RIASEC).<strong>SEGEPLAN pone a tu disposición este test vocacional como una herramienta de acompañamiento. Sus resultados son únicamente orientativos y referenciales, por lo que no condicionan tu elección ni constituyen un requisito obligatorio para tu postulación.</strong></div>
  </div>
...
document.getElementById('showResults').addEventListener('click',async()=>{
  ...
  if(resultSaved){buildResults();alertBox.textContent='Has terminado la orientación vocacional con éxito. Verifica tus resultados.';alertBox.classList.add('show');return;}
  ...
  try{await saveTestResult();resultSaved=true;alertBox.textContent='Has terminado la orientación vocacional con éxito. Verifica tus resultados.';alertBox.style.cssText='display:block;background:#eaf8ee;border-color:#b8dfc3;color:#26703c';}
