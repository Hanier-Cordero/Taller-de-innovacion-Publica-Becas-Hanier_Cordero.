...
require __DIR__.'/../api/config.php'; requireAdmin(); $pdo=db();
$fields=['nombre','tipo','sector','departamento','municipio','direccion','telefono','sitio_web'];
if($_SERVER['REQUEST_METHOD']==='POST'){
 $action=$_POST['accion']??'';$id=(int)($_POST['id']??0);
 if($action==='estado'&&$id){$pdo->prepare('UPDATE centros_estudio SET activo=NOT activo WHERE id=?')->execute([$id]);header('Location: centros.php?actualizado=1');exit;}
 if($action==='guardar'){$v=[];foreach($fields as $f)$v[$f]=trim((string)($_POST[$f]??''));
  if($v['nombre']===''||$v['departamento']===''||$v['municipio']==='')$error='Nombre, departamento y municipio son obligatorios.';
  else try{if($id){$set=implode(',',array_map(fn($f)=>"$f=?",$fields));$pdo->prepare("UPDATE centros_estudio SET $set WHERE id=?")->execute([...array_values($v),$id]);}else{$pdo->prepare('INSERT INTO centros_estudio ('.implode(',',$fields).') VALUES ('.implode(',',array_fill(0,count($fields),'?')).')')->execute(array_values($v));}header('Location: centros.php?guardado=1');exit;}catch(PDOException $e){$error=$e->getCode()==='23000'?'Ya existe un centro con ese nombre y ubicación.':'No fue posible guardar el centro.';}
 }
}
$edit=null;if(isset($_GET['editar'])){$s=$pdo->prepare('SELECT * FROM centros_estudio WHERE id=?');$s->execute([(int)$_GET['editar']]);$edit=$s->fetch();}
